    ...
    <div class="max-w-md w-full space-y-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          Or
          <NuxtLink to="/sign-up" class="font-medium text-primary-600 hover:text-primary-500">
...
const handleSubmit = async () => {
  ...
  try {
    ...
    }>(`${config.public.apiBaseUrl}/auth/sign-in`, {
      method: 'POST',
      body: form.value,
    });
    ...
    error.value = err.data?.error?.message || 'Failed to sign in. Please try again.';
  } finally {
    loading.value = false;
  }
};
